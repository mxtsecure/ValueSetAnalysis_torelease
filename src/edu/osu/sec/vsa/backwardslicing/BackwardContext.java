...
public class BackwardContext extends AbstractStmtSwitch implements StmtPath, ICollecter {
	...
	public void handleInvokeExpr(Value restAssignTo, boolean leftisIntrested, InvokeExpr invokExp) {
		...
		//如果感兴趣的变量位于返回值的位置，则调用diveIntoMethodCall()函数进行获取返回值，从而得到感兴趣变量的值。
		//如果感兴趣的变量位于函数参数的位置或者，则将其
		...
		} else if (mthSig.equals("<scala.collection.mutable.StringBuilder: java.lang.String toString()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<android.net.Uri: android.net.Uri$Builder buildUpon()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<android.net.Uri$Builder: android.net.Uri build()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<android.net.Uri: java.lang.String toString()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<java.lang.String: java.lang.String trim()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<java.net.URL: java.lang.String getHost()>")) {
			// tmp.getBase()
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<android.content.SharedPreferences: java.lang.String getString(java.lang.String,java.lang.String)>")) {
		...
		} else if (mthSig.equals("<scala.collection.mutable.StringBuilder: scala.collection.mutable.StringBuilder append(java.lang.Object)>")) {
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
			if (invokExp.getArg(0) instanceof Local) {
				this.addIntrestedVariable(invokExp.getArg(0));
			}
...
//			}
			this.addIntrestedVariable(((VirtualInvokeExpr) invokExp).getBase());
		} else if (mthSig.equals("<com.yanzhenjie.nohttp.BasicRequest: void buildUrl(java.lang.StringBuilder)>")) {
			if (invokExp.getArg(0) instanceof Local) {
				this.addIntrestedVariable(invokExp.getArg(0));
			}
	...
	//diveIntoMethodCall 只能处理return的函数
